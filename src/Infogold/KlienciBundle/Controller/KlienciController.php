...
use Infogold\KlienciBundle\Form\ProduktyKlientaType;
use Infogold\KlienciBundle\Form\FakturaType;
use Infogold\KlienciBundle\Entity\Faktura;
use Symfony\Component\Form\FormError;
use Infogold\KlienciBundle\Form\KontaktyType;
...
class KlienciController extends Controller {
    ...
    public function showAction(Request $request, $id) {
        ...
        $nrklienta = $user->getFirma()->getNrklienta();
        ...
        $magazyn = $querym->getOneOrNullResult();
        ...
        if ($zakupione) {
            $faktura = new Faktura;
            $formfaktura = $this->createForm(new FakturaType(true), $faktura);
      ...
      public function fakturaAction(Request $request, $id) {
       ...
       // $loggedUser = $this->get('my.main.admin')->getMainAdmin();
        ...
        $loggedUser = $this->get('security.context')->getToken()->getUser()->getFirma();
    ...
    public function zapiszkontaktAction(Request $request, $id) {
        ...
        if ($formkont->isValid()) {
            ...
            $er->flush();
            # zapisz kontakt i przekieruj na stronę show
            return $this->redirect($this->generateUrl('klienci_show', array('id' => $id)) . '#tab_content2');
        }
        return $this->redirect($this->generateUrl('klienci_show', array('id' => $id)) . '#tab_content2');
    }
    ...
    public function fakturapronrAction(Request $request) {
        ...
        if ($request->getMethod() == 'POST') {
            ...
            $id = $request->get('id');           
            $emk = $this->getDoctrine()->getManager();
            $entityk = $emk->getRepository('InfogoldKonsultantBundle:Konsultant')->find($id);
            ...
            $query = $qb
                    ...
                    ->where('c=' . $entityk->getFirma()->getId())
            ...
            $numer = $query->getOneOrNullResult();
            if ($numer) {
